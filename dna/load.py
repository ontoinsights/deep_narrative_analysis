...
from nlp import get_birth_details, parse_narrative
from utilities import resources_root, gender_dict, months, countries, capture_error
    ...
    while True:
        ...
        if event_csv == 'OK':
            if not values['csv_file'] or not values['store_name']:
                ...
                count = _process_csv(selected_csv, selected_store, store_list)
                break
...
def add_narr_data_to_store(narrative: str, narr_metadata: dict, store_name: str):
    ...
    triples_list.append(f':{iri_narrator} a :Person ; rdfs:label "{_get_narrator_names(narr_metadata)}" .')
    ...
    new_triples = _get_birth_details_as_triples(narrative, narr_metadata['Given'], iri_narrator)
    if new_triples:
...
def clean_text(narrative: str, narr_metadata: dict) -> str:
    ...
    """
    new_text = ''
    # Remove the first x line(s) from the converted text
    lines = narrative.split('\n')[int(narr_metadata['Remove']):]
    for line in lines:
        if not line:
...
def _get_birth_details_as_triples(narrative: str, given_name: str, iri_narrator: str) -> list:
    """
    Process the narrative text to see if there is information about where and when
    the narrator/subject was born.
    ...
    new_triples = list()
    birth_date, birth_place = get_birth_details(narrative)
    ...
    if birth_date or birth_place:
        ...
        if birth_place:
            ...
            if not found_country and labels:
                found_country = _get_wikidata_country(labels[0])
            if found_country:
...
def _get_narrator_names(narr_metadata: dict) -> str:
    """
...
def _get_wikidata_country(location: str) -> str:
    """
...
def _process_csv(csv_file: str, store_name: str, store_list: list) -> int:
    """
    ...
    if db_exception:
        capture_error(db_exception, True)
        return 0

    try:
        with open(csv_file, newline='') as meta_file:
            narr_dict = csv.DictReader(meta_file)
            # Process each narrative based on the metadata:
            # Source,Title,Person,Given,Surname,Maiden,Gender,Start,End,Header,Footer
        ...
        unified_triples = _unify_narrators(store_name)
        if unified_triples:
            # Add the triples to the data store
            add_remove_data('add', ' '.join(unified_triples), store_name)
    except Exception as e:
        capture_error(f'Exception ingesting narratives: {str(e)}', False)
    return count
...
def _unify_narrators(store_name: str) -> list:
    """
