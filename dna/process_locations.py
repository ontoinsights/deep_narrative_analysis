...
def check_if_loc_is_known(loc_text: str, plet_dict: dict) -> (list, str):
    """
    ...
    :param plet_dict: A dictionary holding the persons, locations, events & times encountered in
             ...
             Keys = 'persons', 'locs', 'events', 'times' and Values (for 'locs') = array of arrays
    ...
    if 'locs' not in plet_dict:
        return [], empty_string
    known_locs = plet_dict['locs']
...
def get_location_iri_and_ttl(loc_text: str, plet_dict: dict, from_ner: bool,
    ...
    if from_ner:
        ...
        if proper_noun:
            ...
            proper_noun_map, proper_noun_iri = check_if_loc_is_known(proper_noun, plet_dict)
        ...
        new_loc_arrays.append([alt_names, 'LOC', loc_maps, loc_iri])
    # Record location text in plet_dict so that the details are not added again
    if new_loc_arrays:
        known_locs = plet_dict['locs'] if 'locs' in plet_dict else []
        known_locs.extend(new_loc_arrays)
        plet_dict['locs'] = known_locs
    return alt_names, loc_maps, loc_iri, loc_ttl


def get_sentence_locations(sentence_dictionary: dict, plet_dict: dict, last_nouns: list, use_sources: bool) -> list:
    """
    ...
    :param use_sources: Boolean indicating whether additional information on the location should
                        be retrieved from GeoNames (recommended)
    :return: An array of Turtle statements defining new locations identified in the sentence
    ...
    for new_loc in new_locs:
        new_locs_dict[new_loc] = check_if_loc_is_known(new_loc, plet_dict)
    for new_loc, loc_details in new_locs_dict.items():
        loc_maps, loc_iri = loc_details
        if not loc_iri:
            # Need to define the Turtle for a new location
            alt_names, loc_maps, loc_iri, loc_turtle = get_location_iri_and_ttl(new_loc, plet_dict, True, use_sources)
